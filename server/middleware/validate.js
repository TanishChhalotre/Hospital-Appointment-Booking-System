...
// ── OTP schemas ───────────────────────────────────────────────────────────────
// verifyOtpSchema: used for POST /api/auth/verify-otp
// resendOtpSchema: used for POST /api/auth/resend-otp
const verifyOtpSchema = z.object({
  email,
  // otp must be exactly 6 digits — .regex() checks the pattern
  otp: z.string().trim().regex(/^\d{6}$/, 'OTP must be exactly 6 digits'),
}).strict();

const resendOtpSchema = z.object({ email }).strict();
